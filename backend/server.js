...
⚡ СТРОГАЯ СТРУКТУРА ОТВЕТА (ТОЧНО 270 СИМВОЛОВ):
1. ОСНОВНОЙ ТЕКСТ (80 символов) - яркая реакция на заклинание с эмодзи: "Вау! Вот это заклинание! Монстру придется не хило!" или "Интересное заклинание, посмотрим что получится!" БЕЗ УПОМИНАНИЯ УРОНА, только реакция и атмосфера
2. СПИСОК МЕТРИК в формате:
   - 💜 У монстра осталось [ТОЧНО ТО КОЛИЧЕСТВО ЧТО ПЕРЕДАНО В ПРОМПТЕ] жизней!
   - 🎯 У игрока осталось [ТОЧНО ТО КОЛИЧЕСТВО ЧТО ПЕРЕДАНО В ПРОМПТЕ] попыток!
3. ПРИЗЫВ К ДЕЙСТВИЮ (100 символов) с эмодзи

КРИТИЧЕСКИ ВАЖНО: 
- ОТВЕТ ДОЛЖЕН БЫТЬ ТОЧНО 270 СИМВОЛОВ (не больше, не меньше)
- В ОСНОВНОМ ТЕКСТЕ НЕ УПОМИНАЙ УРОН - только реакция на заклинание типа "Вау! Вот это заклинание! Монстру придется не хило!" или "Интересное заклинание, посмотрим что получится!" или "Мощное заклинание! Монстр дрогнул!" или "Слабое заклинание, монстр смеется!"
- УРОН УКАЗЫВАЙ ТОЛЬКО В СПИСКЕ МЕТРИК
...
- Используй РАЗНЫЕ эмодзи каждый раз для эмоций: ⚔️💥🔥💜🎯⚡️✨🗡️👹🛡️💀⚔️🌟💫🔥⚡️💥🎭🎪🎨🎬🎮🎲🎯🏹🗡️⚔️🛡️💀👹👺🤖👽👾💎💍🔥💥⚡️✨🌟💫⭐🌙☀️🌈
- СЧИТАЙ СИМВОЛЫ И ДОБАВЛЯЙ/УБИРАЙ ТЕКСТ ДЛЯ ТОЧНО 270 СИМВОЛОВ`;

    let userPrompt = `Игрок произнес заклинание: "${originalText}"`;

    // Добавляем ТОЧНЫЕ игровые данные в промпт
    if (playerData) {
      // Рассчитываем урон монстру (если игрок нанес урон)
      const damageToMonster = livesLost > 0 ? Math.floor(livesLost * 0.3) : 0; // 30% от урона игрока идет в урон монстру
      
      userPrompt += `\n\nТОЧНЫЕ ДАННЫЕ ДЛЯ ОТВЕТА:
- Урон монстру = ${damageToMonster}
- Жизней осталось ${playerData.lives_count}
- Попыток осталось ${playerData.attempts_left}
- Урон игрока в этом ходу: ${livesLost}`;
    }
    ...
    userPrompt += `\n\nСгенерируй яркий и эмоциональный ответ в стиле фэнтези-игры, используя ТОЛЬКО переданные выше данные. В ОСНОВНОМ ТЕКСТЕ НЕ УПОМИНАЙ УРОН - только реакция на заклинание типа "Вау! Вот это заклинание! Монстру придется не хило!" или "Интересное заклинание, посмотрим что получится!" УРОН УКАЗЫВАЙ ТОЛЬКО В СПИСКЕ МЕТРИК! Будь креативным, используй РАЗНЫЕ эмодзи каждый раз, создавай уникальную атмосферу! ОТВЕТ ДОЛЖЕН БЫТЬ ТОЧНО 270 СИМВОЛОВ! Считай символы и корректируй текст для точного соответствия.`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      max_tokens: 250, // Уменьшаем для соответствия лимиту в 270 символов
    ...
    // Приводим к точно 270 символам
    if (generatedText.length > 270) {
      generatedText = generatedText.substring(0, 270);
      console.log('✂️ Обрезан до 270 символов');
    } else if (generatedText.length < 270) {
      const padding = ' '.repeat(270 - generatedText.length);
      generatedText = generatedText + padding;
      console.log('➕ Дополнен пробелами до 270 символов');
    }
