...
⚡ СТРОГАЯ СТРУКТУРА ОТВЕТА (от 270 до 320 СИМВОЛОВ):
...
3. ПРИЗЫВ К ДЕЙСТВИЮ и написать продолжай играть чтобы выиграть СЕТ (до 100 символов с эмодзи)

КРИТИЧЕСКИ ВАЖНО: 
- ОТВЕТ ДОЛЖЕН БЫТЬ ТОЧНО 320 СИМВОЛОВ (не больше, не меньше)
...
- СЧИТАЙ СИМВОЛЫ И ДОБАВЛЯЙ/УБИРАЙ ТЕКСТ ДЛЯ ТОЧНО ОТ 270 ДО 320 СИМВОЛОВ`;
...
- Урон игрока в этом ходу: ${livesLost}`;
    ...
    userPrompt += `\n\nСгенерируй яркий и эмоциональный ответ в стиле фэнтези-игры, используя ТОЛЬКО переданные выше данные. В ОСНОВНОМ ТЕКСТЕ НЕ УПОМИНАЙ УРОН - только опиши заклинание и атмосферу! УРОН УКАЗЫВАЙ ТОЛЬКО В СПИСКЕ МЕТРИК! Будь креативным, используй РАЗНЫЕ эмодзи каждый раз, создавай уникальную атмосферу! ОТВЕТ ДОЛЖЕН БЫТЬ ТОЧНО от 270 до 320 СИМВОЛОВ! Считай символы и корректируй текст для точного соответствия.`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      max_tokens: 250, // Уменьшаем для соответствия лимиту в 320 символов
    ...
    // Приводим к точно 320 символам
    if (generatedText.length > 320) {
      generatedText = generatedText.substring(0, 320);
      console.log('✂️ Обрезан до 320 символов');
    } else if (generatedText.length < 320) {
      const padding = ' '.repeat(320 - generatedText.length);
      generatedText = generatedText + padding;
      console.log('➕ Дополнен пробелами до 320 символов');
    }
