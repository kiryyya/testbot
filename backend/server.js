...
1. ОСНОВНОЙ ТЕКСТ (до 80 символов) - краткое описание заклинания и результата
2. СПИСОК МЕТРИК в формате:
   - Урон монстру = [ТОЧНО ТОТ УРОН ЧТО ПЕРЕДАН В ПРОМПТЕ]
   - Жизней осталось [ТОЧНО ТО КОЛИЧЕСТВО ЧТО ПЕРЕДАНО В ПРОМПТЕ]
   - Попыток осталось [ТОЧНО ТО КОЛИЧЕСТВО ЧТО ПЕРЕДАНО В ПРОМПТЕ]
3. ПРИЗЫВ К ДЕЙСТВИЮ (краткий, до 20 символов)
...
- Если урон = 0, пиши "Урон монстру = 0"
- Если урон > 0, пиши точное число из промпта
- Основной текст должен быть очень кратким
- Все метрики выноси в список
...
- Урон игрока в этом ходу: ${livesLost}`;
    ...
    userPrompt += `\n\nСгенерируй ответ в стиле фэнтези-игры, используя ТОЛЬКО переданные выше данные. ОТВЕТ ДОЛЖЕН БЫТЬ ТОЧНО 270 СИМВОЛОВ! Считай символы и корректируй текст для точного соответствия.`;
